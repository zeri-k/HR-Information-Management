...
namespace PIM
{
    public partial class DepartmentForm : Form
    {
        ...
        public DepartmentForm(string Department)
        {
            InitializeComponent();
        ...
        private void DepartmentForm_Load(object sender, EventArgs e)
        {
            try
            {
                string commandString = "select emp_num, emp_name, department, jobposition, join_day1, join_day2, join_day3, " +
                    "hl_edu, cellphone1, cellphone2, cellphone3, E_Mail from pers_info where department='" + Department+"'";
                ...
                while (myReader.Read())
                {
                    dgvDepartmentList.Rows.Add();
                    dgvDepartmentList["Col1", i].Value = i + 1;
                    dgvDepartmentList["Col2", i].Value = myReader.GetInt32(0);
                    dgvDepartmentList["Col3", i].Value = myReader.GetString(1);
                    dgvDepartmentList["Col4", i].Value = myReader.GetString(3);
                    dgvDepartmentList["Col5", i].Value = myReader.GetInt32(4).ToString("0000") + "/" + myReader.GetInt32(5).ToString("00") + "/" + myReader.GetInt32(6).ToString("00");
                    dgvDepartmentList["Col6", i].Value = myReader.GetString(7);
                    dgvDepartmentList["Col7", i].Value = myReader.GetInt32(8).ToString("000") + "-" + myReader.GetInt32(9).ToString("0000") + "-" + myReader.GetInt32(10).ToString("0000");
                    dgvDepartmentList["Col8", i].Value = myReader.GetString(11);
        ...
        private void tsmPrint_Click(object sender, EventArgs e)
        {
            ...
            {
                printDocument1.DocumentName = "부서별 사원대장";
        ...
        private void printDocument1_BeginPrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            try
            {
                ...
                foreach (DataGridViewColumn dgvGridCol in dgvDepartmentList.Columns)
                {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            try
            {
                ...
                if (bFirstPage)
                {
                    foreach (DataGridViewColumn GridCol in dgvDepartmentList.Columns)
                    {
                        if (GridCol.Index == 0)
                        {
                        ...
                        else if (GridCol.Index >= 3 && GridCol.Index <= dgvDepartmentList.ColumnCount - 3)
                        {
                ...
                while (iRow <= dgvDepartmentList.Rows.Count - 1)
                {
                    DataGridViewRow GridRow = dgvDepartmentList.Rows[iRow];
                    //Set the cell height
                    ...
                    else
                    {
                        if (bNewPage)
                        {
                            ...
                            // 부서명
                            String strDepart = "부서: " + Department;
