...
namespace PIM
{
    public partial class AllPositionForm : Form
    {
        ArrayList Positionlist = null;

        public AllPositionForm()
        {
            InitializeComponent();
        }
        public AllPositionForm(ArrayList Positionlist)
        {
            InitializeComponent();
        ...
        private void AllPositionForm_Load(object sender, EventArgs e)
        {
            try
            {
                DataGridView[] dataGrids = new DataGridView[7];
                dataGrids[0] = dgvPositionlist1;
                dataGrids[1] = dgvPositionlist2;
                dataGrids[2] = dgvPositionlist3;
                dataGrids[3] = dgvPositionlist4;
                dataGrids[4] = dgvPositionlist5;
                dataGrids[5] = dgvPositionlist6;
                dataGrids[6] = dgvPositionlist7;

                int j = 0;
                int k = 0;
                String[] col = new String[56];
                ...
                foreach (Object ob in Positionlist)
                {
                    string commandString = "select emp_num, emp_name, department, jobposition, join_day1, join_day2, join_day3, " +
                        "hl_edu, cellphone1, cellphone2, cellphone3, E_Mail from pers_info where jobposition='" + ob.ToString() + "'";
                    ...
                    while (myReader.Read())
                    {
                        ...
                        dataGrids[j][col[k + 3], i].Value = myReader.GetString(2);
        ...
        private void tsmPrint_Click(object sender, EventArgs e)
        {
            ...
            {
                printDocument1.DocumentName = "전체 직위별 사원대장";
                ...
                for (; i < dataGrids.Length; i++)
                {
                    ...
                    while (iRow <= dataGrids[i].Rows.Count - 1)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                // 머리글
                                e.Graphics.DrawString("직위별 사원대장", myFont,
                                ...
                                e.Graphics.DrawString(strDate, myFont1,
                                        ...
                                        e.MarginBounds.Top - e.Graphics.MeasureString("직위별 사원대장", myFont1, e.MarginBounds.Width).Height - 16);
                                ...
                                e.Graphics.DrawString(strPagenum, myFont1,
                                        Brushes.Black,
                                        e.MarginBounds.Right - e.Graphics.MeasureString(strDate, myFont1, e.MarginBounds.Width).Width + 10,
                                        e.MarginBounds.Top - e.Graphics.MeasureString("직위별 사원대장", myFont1, e.MarginBounds.Width).Height);

                                // 직위명
                                String strDepart = "직위: " + Positionlist[i];
        ...
        private void AllPositionForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
        }
    }
